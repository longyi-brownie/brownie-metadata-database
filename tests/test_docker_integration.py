...
class TestDockerStackIntegration:
    ...
    def test_postgres_schema(self, docker_stack):
        ...
        expected_tables = [
            ...
            "agent_configs", "stats", "configs", "alembic_version"
        ]
    ...
    def test_prometheus_scraping(self, docker_stack):
        ...
        metrics_target = next(
            (t for t in targets_data if "metrics-sidecar" in t.get("labels", {}).get("job", "")),
            None
        )
    ...
    def test_backup_service(self, docker_stack):
        ...
        assert result.returncode == 0
        assert "Backup completed successfully" in result.stdout or "backup" in result.stdout.lower()
    ...
    def test_migration_completed(self, docker_stack):
        ...
        assert "exited (0)" in result.stdout  # Migration should have completed successfully
        ...
        assert result.returncode == 0
        assert "d607e412e7b0" in result.stdout  # Should have the initial migration applied

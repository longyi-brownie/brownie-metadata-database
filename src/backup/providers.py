...
class BackupProvider(ABC):
    ...
    @abstractmethod
    def upload_backup(self, backup_name: str, backup_path: str, metadata: Dict[str, Any]) -> Dict[str, Any]:
...
class LocalProvider(BackupProvider):
    ...
    def upload_backup(self, backup_name: str, backup_path: str, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info("Backup uploaded successfully", 
    ...
    def download_backup(self, backup_name: str, destination_path: str) -> bool:
        ...
        try:
            ...
            if compressed_path.exists():
                if self.config.compression:
                    return self.decompress_backup(str(compressed_path), destination_path)
                else:
        ...
        try:
            ...
            for file_path in self.backup_dir.glob("*.sql*"):
                ...
                backup_info = {
                    ...
                    "created": metadata.get("created", datetime.fromtimestamp(file_path.stat().st_mtime).isoformat()),
    ...
    def delete_backup(self, backup_name: str) -> bool:
        ...
        try:
            ...
            if deleted:
                ...
                self.logger.warning("Backup not found for deletion", backup_name=backup_name)
                return False
...
class S3Provider(BackupProvider):
    ...
    def __init__(self, config):
        ...
        try:
            ...
            raise RuntimeError("boto3 package required for S3 provider. Install with: pip install boto3")
        except Exception as e:
    ...
    def upload_backup(self, backup_name: str, backup_path: str, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            s3_key = f"{self.prefix}/{backup_name}{file_extension}" if self.prefix else f"{backup_name}{file_extension}"
            ...
            # Upload metadata
            metadata_key = f"{self.prefix}/{backup_name}.json" if self.prefix else f"{backup_name}.json"
            ...
            self.logger.info("Backup uploaded to S3 successfully", 
                           backup_name=backup_name, 
                           size=file_size,
                           bucket=self.bucket_name,
                           key=s3_key)
    ...
    def download_backup(self, backup_name: str, destination_path: str) -> bool:
        ...
        try:
            # Try compressed first, then uncompressed
            compressed_key = f"{self.prefix}/{backup_name}.sql.gz" if self.prefix else f"{backup_name}.sql.gz"
            uncompressed_key = f"{self.prefix}/{backup_name}.sql" if self.prefix else f"{backup_name}.sql"
            ...
            try:
                ...
                try:
                    self.s3_client.head_object(Bucket=self.bucket_name, Key=uncompressed_key)
            ...
            self.logger.info("Backup downloaded from S3 successfully", 
                           backup_name=backup_name,
                           key=s3_key)
            return True
            ...
            for page in page_iterator:
                ...
                for obj in page['Contents']:
                    ...
                    backup_info = {
                        "name": backup_name,
                        "size": obj['Size'],
                        "created": metadata.get("created", obj['LastModified'].isoformat()),
                        "status": metadata.get("status", "unknown"),
    ...
    def delete_backup(self, backup_name: str) -> bool:
        ...
        try:
            ...
            if deleted:
                self.logger.info("Backup deleted from S3 successfully", backup_name=backup_name)
                return True
            else:
                self.logger.warning("Backup not found for deletion in S3", backup_name=backup_name)
                return False
